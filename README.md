...
| `certificates.pdf` | грамоты: три места в абсолюте и в каждой возрастной группе плюс три пустых бланка |
...
| `trophy-*.stl` | кубки победителям — разделочный велосипед на подставке |
...
- [OpenSCAD](https://openscad.org/) — только для сборки STL-моделей кубков.
  Нужна сборка 2023 года или новее: в ней есть движок **manifold**, который
  режет гравировку логотипа за секунды. Версия 2021.01 из репозиториев тоже
  сработает, но потратит на те же файлы полчаса — программа предупредит.
...
Правила работы с репозиторием собраны в [CLAUDE.md](CLAUDE.md) и в скиллах
[.claude/skills/](.claude/skills/): как выкатывать изменения и релизы, из чего
состоит комплект, как устроена модель кубка, как считаются призовые, откуда
берутся логотипы и что известно про сайт команды. В `.claude/hooks/` лежат
два хука: один форматирует записанный python-файл, второй не даёт закоммитить
сообщение со следами соавторства. Личные разрешения (`settings.local.json`)
...
src/ubt_race_docs/
    ...
    assets/models/     trophy.scad — параметрическая модель кубка
                       ubt-logo.svg — контур эмблемы для гравировки
tests/                 тесты
