...
#
# Разбор текстовый, по двоеточиям, а не через VALUE: Apple Numbers,
# в отличие от Excel и LibreOffice, не умеет читать «0:34:12» этой функцией,
# и вся таблица молча оставалась пустой.
#
#   «0:34:12» — часы, минуты, секунды;
...
COLUMNS: tuple[Column, ...] = (
    ...
    Column(
        ...
        10,
        SECONDS_FORMAT,
        '=IF($D{row}="","",IFERROR(ROUND('
        "IF($L{row}=2,VALUE(LEFT($D{row},$M{row}-1))*3600"
        "+VALUE(MID($D{row},$M{row}+1,$N{row}-$M{row}-1))*60"
        "+VALUE(MID($D{row},$N{row}+1,10)),"
        "IF($L{row}=1,VALUE(LEFT($D{row},$M{row}-1))*60"
        "+VALUE(MID($D{row},$M{row}+1,10)),"
        "IF($L{row}=0,VALUE($D{row}),"
